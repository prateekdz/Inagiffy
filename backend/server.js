...
async function generateMapLLM(topic, level) {
...
app.post('/generate', async (req, res) => {
  ...
  const llmResult = await generateMapLLM(topic, level);
  if (llmResult) {
    return res.json({ root: llmResult, source: 'llm' });
  }
  const local = generateMapLocal(topic, level);
  return res.json({ root: local, source: 'local' });
});

app.get('/', (req, res) => res.send('Inagiffy backend is running'));
